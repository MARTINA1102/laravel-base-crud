...
class ComicTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($comics as $comic){
            $comic = new Comic();
            $comic->id();
            $comic->string('title', 100);
            $comic->string('description', 500);
            $comic->string('thumb', 200);
            $comic->unsignedMediumInteger('price')->nullable();
            $comic->string('series', 100);
            $comic->date('sale_date');
            $comic->string('type', 100);
            $comic->timestamps();
            $comic->save();
        }
    }
